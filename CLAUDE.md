...
| Path | Package | Role |
| --- | --- | --- |
| `packages/core` | `@open-doc/core` | Runtime (document browser, page viewer, outline, themes, assets panel, design panel, PDF/HTML export), Vite plugins, dev API, `open-doc` dev/build CLI, canonical skills. |
| `packages/cli` | `@open-doc/cli` | `npx @open-doc/cli init` scaffolder + project template. |
| `packages/mcp` | `@open-doc/mcp` | MCP server exposing the `ops` layer as tools over Streamable HTTP. Opt-in; mounted at `/mcp` by `open-doc dev --mcp`. |
| `apps/demo` | private | Local consumer of `@open-doc/core` via `workspace:*`. Dogfood target — `pnpm dev:demo`. |
...
- **Two copies of core exist at runtime.** The viewer imports `src/app/**`; a document imports the built `dist` bundle via `@open-doc/core`. Anything that must be *shared* between them (React context, the outline store) is stashed on `globalThis` — see `src/app/lib/page-context.tsx` and `src/app/lib/outline.ts`. A new shared singleton must follow the same pattern or it will silently split in two.
...
- **`@open-doc/mcp` is imported dynamically and is not a core dependency.** `mcp-plugin.ts` resolves it through a variable specifier — core must not take a build-time dependency on a package whose peer is core. A missing install warns and disables the endpoint; it is never fatal.
