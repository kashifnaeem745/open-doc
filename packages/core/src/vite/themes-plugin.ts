...
export function parseFrontmatter(raw: string, themeId: string): { fm: Frontmatter; body: string } {
...
export function generateThemesModule(themes: ParsedTheme[], isDev: boolean): string {
...
export function themesPlugin(opts: ThemesPluginOptions): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // Watch the path even when it is missing — a workspace scaffolded without
      // a themes/ directory would otherwise need a restart after its first theme.
