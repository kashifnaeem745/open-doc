...
- **CLI** — `open-doc dev | build | preview | sync:skills`。`dev --mcp` 另會把 [`@open-doc/mcp`](../mcp) 掛在 `/mcp`。
- **Ops 層** — `@open-doc/core/ops`：與傳輸無關的文件、theme、asset 與資料夾操作。Dev API 與 MCP server 都呼叫這一層，因此驗證與衝突檢查只存在一份。
